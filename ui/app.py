...
from feature_extraction.extract_features_resnet import load_resnet50, get_features_from_resnet
...
def retrieve_image(img_query, feature_extractor, n_imgs=11):
    ...
    elif feature_extractor == 'Extractor 2 (resnet)':
        indexer = faiss.read_index(os.path.join(DB_PATH, 'feat_extract_resnet.index'))
        feature_extractor, model = load_resnet()
        img_features = get_features_from_resnet(img_query, model, feature_extractor)    
    elif feature_extractor == 'Extractor 3 (tuned_resnet)':
    ...
    elif feature_extractor == 'Extractor 4 (hog)':
...
def main():
    ...
    with col1:
        ...
        option = st.selectbox('.', ('Extractor 1', 'Extractor 2', 'Extractor 3'))
